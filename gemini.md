...
2.  **📖 Creative Storyteller (Out of Scope):** Transforms physical objects seen by the camera into vivid, interleaved narratives accompanied by AI-generated storyboard images. _Note: Disabled for current release._
...
_Note: `bun run backend:dev` is a convenience script in the root `package.json` that executes `uv run uvicorn main:app` within the `/backend` directory._
