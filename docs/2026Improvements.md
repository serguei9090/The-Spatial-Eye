...
- **Deployment:** Deploy the backend on Google Cloud Run, as recommended in the _Way Back Home_ codelab.
...
- Use **Agent-as-a-Tool** routing logic (from _Way Back Home Level 4_) to delegate tasks dynamically to specialized sub-agents (e.g., an `Architect Agent` for IT diagrams, a `Director Agent` for the Storyteller stream).
...
**The Improvement:**
Adopt the **Memory Bank** and **Graph RAG** patterns from the _Survivor Network_ codelab.
...
**The Improvement:**
The _Survivor Network_ codelab highlights moving away from brittle, direct media pumping toward an agentic sequential pipeline.
